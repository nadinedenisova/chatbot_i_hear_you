...
from schemas.entity import (
    ContentCreate, MenuNodeOut, MenuNodeCreate, MenuNodeUpdate, Message,
    RatingCreate, RatingOut, AllMenuNodeOut,
)
...
@router.get("/", summary="Получить всё дерево меню навигации", response_model=AllMenuNodeOut)
async def get_full_menu(menu_service: MenuService = Depends(get_menu_service)):
    return await menu_service.get_full_menu()


@router.get("/search-by-name", summary="Получить узел меню по названию", response_model=MenuNodeOut)
...
@router.get("/root", summary="Получить корень узла меню навигации", response_model=MenuNodeOut)
...
@router.get("/{menu_id}", summary="Получить узел меню по id", response_model=MenuNodeOut)
...
@router.post("/{menu_id}/content/add", summary="Добавить контент к узлу меню", response_model=Message)
...
@router.put("/{menu_id}/content/change/{content_id}", summary="Обновить контент", response_model=Message)
...
@router.delete("/{menu_id}/content/delete/{content_id}", summary="Удалить контент", response_model=Message)
...
@router.get("/{menu_id}/rate", summary="Получить рейтинг узла меню", response_model=RatingOut)
